...
| T9 发布准备(README/pyproject 完善)+ 远程发布 | P7 | 主 agent + 用户 | DOING | 发布准备完成(2026-07-29):LICENSE 为标准 MIT;pyproject 元数据独立核对一致,Homepage 已改指真实仓库(私有);README 发布状态表述已按复验意见消歧。**公开=一条命令**:`gh repo edit deanjo/canonmark --visibility public --accept-visibility-change-consequences`(gh 2.96 起需该 flag,复验实测)——公开动作是红线,留用户执行 |
...
- **波次 3(DONE,复验判定 ACCEPT):T13–T14 读取接线**。顺序不可与波次 2 颠倒——理由见 roadmap「依赖顺序」。
  ...
  **修复后复验判定 ACCEPT**(附两项必办,已完成并入 commit 6e21f78)。acceptance 的 A17/A18 已按用户裁决(2026-07-29)由 INSUFFICIENT_EVIDENCE 置 PASS;审读修复(可信度收尾)已完成落盘,当前推进决策卡执行——波次 4 之前的最后一道门。
