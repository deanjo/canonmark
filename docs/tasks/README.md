...
| T13 `canon_read` 行为契约 + `canon index` 紧凑输出与过滤 | P6 | 实现 agent | NEEDS-VERIFY | `src/canonmark/read.py` + `index.py`,CLI `canon read` / `canon index --dir/--current-only/--json`。作废文档正文不返回(哨兵串断言,不靠人肉检查);正文交付时剥掉 frontmatter(头部摘要已给过,再附一遍等于同样的字节收两次费)。**机器判客观事实,不假装判语义**:按 status/superseded_by 过滤是机器的活,`not_for`/`applies_when` 是否命中当前任务由调用方判断——假装能匹配比不匹配更危险,一次错误的「不适用」会让 agent 跳过真正该读的文档。index 的紧凑性由「大小不随正文变长而增长」守住,比原判据「< 全文 10%」更本质(后者在文档很短时会失真) |
| T14 MCP server + `canon init` 生成接线片段 | P6 | 实现 agent | NEEDS-VERIFY | `src/canonmark/mcp.py`(stdio JSON-RPC,`canon mcp`)+ `canon init --print-mcp`。**手写而非用官方 SDK**:canonmark 承诺 `pip install` 后纯标准库即可跑,MCP 的 stdio 传输就是逐行 JSON-RPC,自己实现不到两百行,换取不把额外依赖树压给使用者;范围限于 initialize/tools/list/tools/call,完整握手有测试。**§7.5/§7.7 的措辞约束已机检**:工具描述必须含「不要在每次读文档前调用」、不得出现「先跑 canon index」,宿主话术必须是 §7.7 原文——把废止过的设计交给人的记性守不住,交给测试 |
...
- **波次 3(待复验):T13–T14 读取接线**。顺序不可与波次 2 颠倒——理由见 roadmap「依赖顺序」。
