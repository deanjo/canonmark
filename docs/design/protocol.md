...
历史导航不算打架:任意层级 `archive/`(含按模块分的 `<模块>/archive/`,目录名不区分大小写)里的索引 README 本职就是列出归档文档;目标路径在 `archive/` 之下的追溯链接,读者不会误当现行入口,同样不报。

同一互检也覆盖现行文档的正文:`status: current` 的非 README 文档链接到 `archive/` 之外的已作废文档即报错(典型场景:总账还指着已被取代的交接卡)。三类不查——来源在 `evidence/` 下(证据只增不改)、来源在 `archive/` 下、接替者链接自己在 `supersedes` 里声明取代的前任(关系已由两端标签声明)。
...
### 7.8 文档体检任务单(`canon doctor`)

`canon doctor [PATH] [--config FILE] [--dir SUBDIR]` 只打印一张交给会话内 AI 的中文任务单,
自身不调用 AI、不联网、不改文件。任务单含三块:写作规范指针;所选范围的完整目录、Markdown
行数与标签事实,加上始终覆盖全库的 `canon audit --all` 结果;只读诊断流程与固定输出格式。

`--dir` 只缩小目录树和正文盘点范围,不改变全库门禁范围。任务单不承载写作规范正文:
优先要求加载 `docs-standard` skill,没有该 skill 时才读取配置项 `standard` 指向的仓库相对路径或
URL;两者都不可用时报告阻塞,不得猜规则。退休文档只列路径与标签,并明确禁止读取正文。
若 audit 的 issue / notice 定位到退休路径,doctor 保留 gate 结果、计数、类型、路径与行号,但把说明
固定替换为「退休文档产生的 audit 明细已隐藏（gate 结果、路径与行号保留）」。这层脱敏只属于
doctor 的任务单展示;直接运行 `canon audit` 的输出与 V 系列行为不变,doctor 也不提供恢复原文参数。

合法启动后,文档问题、单篇 frontmatter 解析失败或 audit FAIL 都不改变 exit 0;PATH、`--dir`
不存在或越界,以及显式配置缺失、损坏或类型错误,沿用 CLI 调用错误的非零退出语义。任务单不设
事实条数上限,大库须由调用方显式用 `--dir` 分批,单次所选范围不得静默截断。
