...
钩子已经随仓提供:`canon hook` 实现了 PreToolUse 协议,拒绝对 `docs/` 下作废
文档的内置 `Read` 或已识别的 `Bash` 读取,并给出与 `canon read` 相同的替代
指针;`canon init --print-hook` 会打印接线用的 `settings.json` 片段。在支持钩子
的宿主上,已覆盖的读取路径从「过滤」升级成了「强制」。拒绝结果还要求 agent
在最终回复里转告替代去处;这仍是需要行为实测的模型可见指引,不是运行时强制。
退休文档没有声明替代目标时,hook 会从目标目录向上到 docs 根,给出第一个真实存在
的 `README.md`。仍然存在的边界,如实写在这里:Python / Perl / xxd 读取与 `cd`
后的相对路径漂移不拦截;而解析失败一律静默放行,因为一个坏掉的闸机绝不能把
整座图书馆锁上。
